...
class TextLSTM(nn.Module):
    def __init__(self):
        ...
        #第一层
        self.Wi = nn.Linear(emb_size + n_hidden, n_hidden, bias=True)
        self.Wf = nn.Linear(emb_size + n_hidden, n_hidden, bias=True)
        self.Wg = nn.Linear(emb_size + n_hidden, n_hidden, bias=True)
        self.Wo = nn.Linear(emb_size + n_hidden, n_hidden, bias=True)

        #第二层
        self.Wi2 = nn.Linear(n_hidden + n_hidden, n_hidden, bias=True)
        self.Wf2 = nn.Linear(n_hidden + n_hidden, n_hidden, bias=True)
        self.Wg2 = nn.Linear(n_hidden + n_hidden, n_hidden, bias=True)
        self.Wo2 = nn.Linear(n_hidden + n_hidden, n_hidden, bias=True)
    ...
    def forward(self, X):
        ...
        list_C = torch.tensor(np.zeros((batch_size, n_hidden))).to(torch.float32)
        list_h = torch.tensor(np.zeros((batch_size, n_hidden))).to(torch.float32)

        list_C2 = torch.tensor(np.zeros((batch_size, n_hidden))).to(torch.float32)
        list_h2 = torch.tensor(np.zeros((batch_size, n_hidden))).to(torch.float32)

        for i in range(1, n_step + 1):
            X1 = torch.cat([X[i - 1, :, :], list_h], dim=1)
            it = torch.sigmoid(self.Wi(X1))
            ft = torch.sigmoid(self.Wf(X1))
            gt = torch.tanh(self.Wg(X1))
            ot = torch.sigmoid(self.Wo(X1))
            list_C = ft.mul(list_C) + it.mul(gt)
            list_h = ot.mul(torch.tanh(list_C))

            X2 = torch.cat([list_h,list_h2], dim=1)
            it = torch.sigmoid(self.Wi2(X2))
            ft = torch.sigmoid(self.Wf2(X2))
            gt = torch.tanh(self.Wg2(X2))
            ot = torch.sigmoid(self.Wo2(X2))
            list_C2 = ft.mul(list_C2) + it.mul(gt)
            list_h2 = ot.mul(torch.tanh(list_C2))

        model = self.W(list_h2.to(torch.float32))  # model : [batch_size, n_class]
        return model

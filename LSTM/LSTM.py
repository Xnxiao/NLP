...
class TextLSTM(nn.Module):
    ...
    def forward(self, X):
        ...
        for i in range(1,n_step+1):
            X1=torch.cat([X[i-1,:,:],list_h],dim=1)#X1:[batch_size,emb_size+n_hidden]
            it = torch.sigmoid(self.Wi(X1))        #输入门
            gt = torch.tanh(self.Wg(X1))
            ft = torch.sigmoid(self.Wf(X1))        #遗忘门
            ot = torch.sigmoid(self.Wo(X1))        #输出门
            list_C= ft.mul(list_C)+ it.mul(gt)     #记忆更新
            list_h= ot.mul(torch.tanh(list_C))     #输出

        model = self.W(list_h.to(torch.float32))# model : [batch_size, n_class]
        return model
